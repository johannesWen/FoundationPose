...
class FoundationPose:
    ...
    def reset_object(self, model_pts, model_normals, symmetry_tfs=None, mesh=None):
        ...
        logger.info(f"self.pts:{self.pts.shape}")
        self.mesh_path = None
        self.mesh = mesh
    ...
    def to_device(self, s="cuda:0"):
        for k in self.__dict__:
            ...
            if torch.is_tensor(self.__dict__[k]) or isinstance(self.__dict__[k], nn.Module):
                logger.info(f"Moving {k} to device {s}")
    ...
    def make_rotation_grid(self, min_n_views=40, inplane_step=60):
        ...
        logger.info(f"cam_in_obs:{cam_in_obs.shape}")
        rot_grid = []
        ...
        logger.info(f"rot_grid:{rot_grid.shape}")
        rot_grid = mycpp.cluster_poses(30, 99999, rot_grid, self.symmetry_tfs.data.cpu().numpy())
        rot_grid = np.asarray(rot_grid)
        logger.info(f"after cluster, rot_grid:{rot_grid.shape}")
        self.rot_grid = torch.as_tensor(rot_grid, device="cuda", dtype=torch.float)
        logger.info(f"self.rot_grid: {self.rot_grid.shape}")
    ...
    def guess_translation(self, depth, mask, K):
        ...
        if len(us) == 0:
            logger.info(f"mask is all zero")
            return np.zeros((3))
        uc = (us.min() + us.max()) / 2.0
        vc = (vs.min() + vs.max()) / 2.0
        valid = mask.astype(bool) & (depth >= 0.001)
        if not valid.any():
            logger.info(f"valid is empty")
            return np.zeros((3))
    ...
    def register(self, K, rgb, depth, ob_mask, ob_id=None, glctx=None, iteration=5):
        ...
        if valid.sum() < 4:
            logger.info(f"valid too small, return")
            pose = np.eye(4)
        ...
        logger.info(f"after viewpoint, add_errs min:{add_errs.min()}")
        ...
        add_errs = self.compute_add_err_to_gt_pose(poses)
        logger.info(f"final, add_errs min:{add_errs.min()}")

        ids = torch.as_tensor(scores).argsort(descending=True)
        logger.info(f"sort ids:{ids}")
        scores = scores[ids]
        poses = poses[ids]
    ...
    def track_one(self, rgb, depth, K, iteration, extra={}):
        if self.pose_last is None:
            logger.info("Please init pose by register first")
            raise RuntimeError
        logger.info("Welcome")
        ...
        depth = erode_depth(depth, radius=2, device="cuda")
        depth = bilateral_filter_depth(depth, radius=2, device="cuda")
        logger.info("depth processing done")
